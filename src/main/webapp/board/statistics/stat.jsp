...
<div class="panel" align="center">
        ...
        <form class="form-horizontal" id="custom_form">
            ...
            <div class="row">
                <div class="control-group span8" style="width: 80%">
                    ...
                    <button class="hide" style="float:left;position: relative;left:4%;top:20%" id="addCourtBtn">添加法院</button>
                </div>
            </div>
            <hr/>
            <%}%>
    ...
    <div id="addCourtDiv_c" class="hide">
        <form class="form-horizontal" id="addCourt_form">
            ...
            <div id="btnDiv" style="float:left;position:relative;left:13%;height: 100%;width: 10%;">
                <button style="margin: 10px" id="addCourtLiBtn">添加</button>
                <button style="margin: 10px" id="removeCourtLiBtn">删除</button>
                <button style="margin: 10px" id="removeAllCourtBtn">清空</button>
            </div>
    ...
    <div class="panel-body" style="height: 400px;padding-top: 100px">
        <div style="width:350px;text-align: left" id="a_select">
        ...
        <br>
        <br>
        <a href="#">7、法官等级情况统计表之七</a>
        <%--<br>--%>
        <%--<br>--%>
        <%--<a href="#">8、法院工作人员统计表之一</a>--%>
        <br>
        <br>
        <a href="#">8、法院工作人员统计表之八（人员分类）</a>
        <br>
        <br>
        <a href="#">9、法院工作人员统计表之九（法官助理、书记员等级信息）</a>
            <br>
            <br>
            <a href="#">10、法院工作人员统计表之十（刑事、民事、执行、立案、行政 部门下的法官数量统计报表）</a>
        </div>
    </div>
    </div>
<script>
    ...
    $("#a_select a").each(function(index,_this){
        ...
        else if(index == 3)
        {
            $(_this).click(function(){
                var returnStr = getAttachName();
                location.href = "ryStat2.jsp?type=fss_01_4" + returnStr;
            })
        }
        else if(index == 4)
        {
        ...
        else if(index == 6)
        {
            $(_this).click(function(){
                var returnStr = getAttachName();
                location.href = "statisticalTableOfJudges.jsp?type=fss_01_7" + returnStr;//法官等级情况统计表
            })
        }
        // else if(index == 7)
        // {
        //     $(_this).click(function(){
        //         var returnStr = getAttachName();
        //         location.href = "StatisticalTableOfStaff.jsp?type=fss_01_8" + returnStr;//法院工作人员职级统计表
        //     })
        // }
        else if(index == 7)
        {
            $(_this).click(function(){
                var returnStr = getAttachName();
                location.href = "ryStat.jsp?type=fss_01_9" + returnStr;//法院工作人员职级统计表()
            })
        }
        else if(index == 8)
        {
            $(_this).click(function(){
                var returnStr = getAttachName();
                location.href = "ryStat2.jsp?type=fss_01_10" + returnStr;//法官助理、书记员 等级统计表
            })
        }
        else if(index == 9)
        {
            $(_this).click(function(){
                var returnStr = getAttachName();
                location.href = "ryStat2.jsp?type=fss_01_11" + returnStr;//法院工作人员统计表之十（增加刑事、民事、执行、立案、行政 部门下的法官数量统计报表）
            })
        }
    })
    function getAttachName()
    {
        ...
        var preparationName_ = $("#preparation")[0].options[$("#preparation")[0].selectedIndex].text;
        var preparationName = preparationName_== "不限制"?"":preparationName_;
        var returnStr = "&courtLevel="+courtLevel+"&attachName="+attachName+"&preparation="+preparation+"&preparationName="+preparationName+"&courtCode="+courtCode;
        return returnStr;
    }
    ...
    BUI.use(['common/search', 'bui/tree', 'bui/data', 'bui/calendar', 'bui/overlay', 'bui/form', 'bui/uploader', 'bui/mask', 'bui/tab'],
            function (Search, Tree, Data, Calendar, Overlay, Form, Uploader, Mask, Tab) {
                    ...
                    buttons: [
                        {
                            ...
                            if(myForm.isValid())
                            {
                                ...
                                if ($('#statForm').length == 0) {
                                    $('body').append($('<form id="statForm" action="ryStat7.jsp" method="post" ></form>'));
                                }
                                var form = $('#statForm');
                                form.hide();
                                form.empty();
                ...
                $("#customSearchBtn").click(function(){
                    ...
                    $.post("<%=basePath%>/code/getCodeWithNotNull", {}, function (data) {
                    ...
                    $("#leftColumn").append($("<option>").attr({"value": 94}).attr({"columnName": "personnel_classification"}).text("人员分类"));
                    $("#leftColumn").select2();
                    $("#topColumn").select2();
