...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        var arr = new Array("学历学位", "单位及职务", "陪审员信息");
//                "家庭信息", "考核信息", "奖励信息", "惩处信息",
//                "出国信息", "外语信息", "交流信息", "司法考试",
//                "专业技术职务", "伤亡信息", "备注信息",
//                "声音与影像", "工资信息", "后备干部", "通讯录",
...
//                        $("#tj option[value='in']").remove();
            ...
            $("[type='submit']").on('click', function () {
                ...
                $("[sub='sub']").each(function () {
                    ...
                    var jurorArray = ["juror_company","juror_jurorWork","juror_regional","juror_number_of_times"
                        ,"juror_type_of_case","juror_member_state"];
                    if(term2q == "enter_src" || term2q == "company_info_id"){
                        c2 = " c."+term2q;
                    }else if($.inArray(term2q,jurorArray) > -1 ){
                        c2 = " c."+term2q;
                    }else{
                        c2 = " a."+term2q;
                    }
                    ...
                    if (c3 == 'is null') {
                        ...
                        } else if(c2 == ' c.juror_type_of_case'){
                            arr = c4.split(',');
                            submit += " " + c1 + " (" ;
                            for (var a = 0;a<arr.length;a++) {
                                submit += c2 + " like  '%"+ arr[a] + "%' ";
                                if(a != arr.length - 1){
                                    submit += " or ";
                                }
                ...
                $("#redirect input[name='userType']").val(3);
                $("#redirect").submit();
            });
    ...
    //列表1选中后出列表2的选项
    function getList2(a) {
        $.post("<%=basePath%>chart/psyGetList2", {list1Id: a}, function (data) {
            $("#list2").empty();
...
<div class="panel">
    <div class="panel-header">
        <h3>陪审员自定义查询 -- <button href="javascript:void(0)" onclick="saveEdit_grid_dialog.show()" class="button button-primary search">点击查询已保存搜索</button></h3>
    </div>
...
<jsp:include page="statisticsList.jsp?userType=3"></jsp:include>
<script>
