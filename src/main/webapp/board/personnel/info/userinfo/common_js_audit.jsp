...
<script>
    ...
    $(function () {
        ...
        BUI.use(["common/search", 'bui/overlay', 'bui/form', 'bui/calendar'], function (Search, Overlay, Form, Calendar) {
            ...
            var isManager = false;
            $.ajax({
                url: "<%=basePath%>subitem/isManager",
                type: "post",
                dataType: "json",
                async: false,
            ...
            if (isManager) {
                items.push(
                    {
                        text: '<i class="icon-info-sign"></i>审核信息<span style="color:red"> ( <span id="dshCount"></span> )</span>',
                        btnCls: 'button button-small',
                        handler: auditInfo
                    }
                )
...
//            dialog.set('height', dialogModHeight);
            ...
            //添加 名字为 qcsfre_required的校验规则 qcsfre：清除是否入额
            Form.Rules.add({
                name: 'qcsfre_required', // 规则名称 qcsfre：清除是否入额
                msg: '有退额信息，该项不应为是', // 默认显示的错误信息
                //value：yefg_end_time的val值 baseValue：true  formatMsg：提示信息
                validator: function (value, baseValue, formatMsg) {
                    // 如果退出员额原因不为空,那么退出员额时间必填
                    if ($("#yefg_end_reason").val() != "" || $("#yefg_end_time").val() != "") {
                        if (baseValue && value.trim() == "1") {
                            return formatMsg;
                        }
                    }
                }
            ...
            //添加 名字为 resj_required的校验规则 resj_required：入额时间
            Form.Rules.add({
                name: 'resj_required',
                msg: '填写了退额信息，该项可不填', // 默认显示的错误信息
                //value：yefg_end_time的val值 baseValue：true  formatMsg：提示信息
                validator: function (value, baseValue, formatMsg) {
                    // 如果退出员额原因不为空,那么退出员额时间必填
                    if ($("#is_yefg").val() == "1" && value.trim() == "" && baseValue && $("#yefg_end_reason").val() == "" && $("#yefg_end_time").val() == "") {
                        return "不能为空";
                    }
                }
            });
            ...
            // 最后加载待审核的数量
            $.post(
                '<%=basePath%>subitem/selectSubItemInfoForBUIOnlyForCount',
                {
