...
<%
    ...
    boolean hasOrgCode = StringUtils.hasText(request.getParameter("orgCode"));
...
<head>
    ...
    <jsp:include page="admin_depart.jsp"></jsp:include>
            ...
            <div class="row">
                ...
                <div class="control-group span8">
                    ...
                    <div class="controls">
                        ...
                        <%--                        <input class="scourt" type="text">--%>
                        <%--                        <select id="courtInput" class="Courtcode search_field" name="innerCourtNo"></select>--%>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    //中央政法编制 、地方事业编制 、纪委派驻人员 类型
    var orgCode = '${param.orgCode}';
    // simpleQuery 精简查询结果 提供查询速度
    var query_condition = {userType: 1, isInfoComplete: 0, isValid: 1, simpleQuery : true };
    if(orgCode){
        query_condition = $.extend({},query_condition, {orgCode: orgCode});
    }
    var canDelete = false;
    ...
    $(function () {
        ...
        $.getJSON("/ums/code/codeListByType", {typeId: 1}, function (data) {
            ...
            for (var i = 0; i < data.data.length; i++) {
                option += '<option value="' + data.data[i].id + '">' + data.data[i].codeName + '</option>';
            }
            $("select.Courtcode").append(option);
            // $("#courtInput").append(option);
    ...
    BUI.use(['common/search', 'bui/tree', 'bui/data', 'bui/calendar', 'bui/overlay', 'bui/form', 'bui/uploader', 'bui/mask', 'bui/grid'], function (Search, Tree, Data, Calendar, Overlay, Form, Uploader, Mask, Grid) {
        ...
        var columns = [
            ...
            {title: '性别', dataIndex: 'genderText', width: "50", sortable: true, align: 'left'},
            {
                ...
                title: '行政职务',
                dataIndex: 'administrationPositionReportText',
                width: "150",
                sortable: true,
                align: 'left'
            },
            {
                title: '法律职务',
                dataIndex: 'lawPositionReportText',
                width: "150",
                sortable: true,
                align: 'left'
            }, {
                title: '职级',
                dataIndex: 'rankReportText',
                width: "100",
                sortable: true,
                align: 'left'
            },
                ...
                title: '操作', width: "300", sortable: false, renderer: function (value, obj) {
                    ...
                    if (obj.leaveReason != null && obj.leaveReason == 5) {
                        ...
                        if (obj.isValid == 1) {
                            ...
                            //3 为仅保留登录
                            spanStr += '<span class="grid-valid  btn-enabled btn-enabled-1" title="申请启用">申请启用</span>' + '<span class="grid-valid  btn-enabled btn-enabled-0" title="申请停用">申请停用</span>';
                        } else {
                            if (<%=equals%>) {
                ...
                },
                {
                    ...
                    handler: function () {
                        ...
                        editDialog.set('headerContent',
                            ...
                            '<li><a href="#userStatus" onclick="changeMarkers(this)">身份信息</a></li>'+
                            '<li><a href="#assessType" onclick="changeMarkers(this)">考核类型</a></li>'+
                            '</ul>');
                        editDialog.show();
                        $("#courtNo").val(CourtNo);
                        $("select.Courtcode").select2();
...
<%--                %>--%>
                ...
                {
                    text: '<i class="icon-inbox "></i>分管部门',
                    btnCls: 'button button-small ',
                    handler: function () {
                        var userinfo = grid.getSelected();
                        if (userinfo) {
                            $("#adminDeptHideId").val(userinfo.id);
                            //设置为只能选择本法院
                            $("#n_courtInfoNo").prop("disabled", false);
                            $("#n_courtInfoNo").val(userinfo.courtCode).change();
                            $("#n_courtInfoNo").prop("disabled", true);
                            adminDepartDialog.set('title', '分管部门 - ' + userinfo.fullname);
                            adminDepartDialog.show();
                            V.courtCode = userinfo.courtCode;
                            V.department = userinfo.department;
                            var datas = {
                                'umsAssignAdminDept.userUuid': userinfo.id
                            };
                            queryAdminDeptInfo(datas);
                        } else {
                            BUI.Message.Alert("请选择一条记录", null, "info");
                        }
                    }
...
<%--                <%    } %>--%>
        ...
        grid.on('cellclick', function (ev) {
            ...
            if (sender.hasClass('btn-enabled')) {
                ...
                if (  (record.isValid != 3 && record.isValid != 1 )|| ( record.isValid == 3 && sender.hasClass('btn-enabled-1')  )   ) {
                    isEdit = true;
            ...
            buttons: [
                {
                    text: '保存', elCls: 'button button-primary', handler: function () {
                        var beforeCourtWorkYear = $("#beforeCourtWorkYear").val()
                        var re = /^\d+$/
                        // var re = /^[1-9]\d*$/;//正整数
                        if(!re.test(beforeCourtWorkYear)){
                            BUI.Message.Alert("入院前工作年限不能为小数", null, "warning");
                            return
                        }
                        //处理防止username被sql注入拦截的问题
        ...
        //----------------------------------------- 分管部门
        var adminDepartForm = new Form.HForm({
            srcNode: '#admin_depart_form',
            submitType: 'ajax',
            callback: function (data) {
                var datas = {
                    'umsAssignAdminDept.userUuid': $("#adminDeptHideId").val(),
                }
                queryAdminDeptInfo(datas);
                if (data.success) {
                    BUI.Message.Alert("添加成功", "success");
                } else {
                    var str = data.msg ? data.msg : "";

                    BUI.Message.Alert("添加失败   " + str, "error");
                }
            }
        }).render();
        adminDeptForm = adminDepartForm;
        var adminDepartDialog = new Overlay.Dialog({
            title: '分管部门',
            buttons: [
                {
                    text: '关闭', elCls: 'button', handler: function () {
            ...
            contentId: 'admin_depart_info', //配置DOM容器的编号
            success: function () {
                this.close();
            }
        })
...
//                        }
        ...
        function reloadUserinfo() {
            $("#userinfo_form [name^='userInfo.']").each(function () {
                if($(this).attr("name") == 'userInfo.assessType') return;
        ...
        function userCheck() {
            ...
            if (myForm.isValid()) {
                ...
                if( ( (  handleUserData && handleUserData.isValid  ) || (!handleUserData  )  )&& $("#leaveReason").val()){
                ...
                // myForm.submit();
                $("#userinfo_form").find(":input[data-rules]").each(function(){
                    var $this = $(this);
                    if($this.attr('data-rules')){
                        var dataRules = $this.attr('data-rules').replace(" ","");
                        if(dataRules.indexOf("required:true") > -1 && !$this.val() && $this.val() !== '0' ){
                            //需要进行验证
                            if($this.is(":hidden") || $this.hasClass("select2-hidden-accessible")){
                                var top = $this.closest(".row").position().top;
                                var btop =  $("#userinfo_form").closest(".bui-stdmod-body").scrollTop();
                                $("#userinfo_form").closest(".bui-stdmod-body").scrollTop(btop + top - 182 );
                                return false;
                            }else{
                                $this.focus();
                                return false;
    ...
    function adminDeptSubmit() {

        if ((V.courtCode != $("#n_courtInfoNo").val()) ) {
            BUI.Message.Alert("操作错误,目前不支持分管其他法院", "warning");
        } else {
            $("#n_courtInfoNo").prop("disabled", false);
            adminDeptForm.valid();
            if (adminDeptForm.isValid()) {
    ...
    function queryAdminDeptInfo(datas) {
        $("#adminDepartMainTable").html('');
        $.post('<%=path%>/adminDept/select', datas, function (data) {
            if (data.list.length > 0) {
                $("#adminDepartMainTable").append(
                    '<TR class="titile_tr tr_1">' +
                    '<TD>姓名</TD>' +
                    '<TD>分管法院</TD>' +
                    '<TD>分管部门</TD>' +
                ...
                for (var i in data.list) {
                    if (i % 2 == 1) {
                        $("#adminDepartMainTable").append(
                            ...
                            '<TD>' + data.list[i].adminCourtName + '</TD>' +
                            '<TD>' + data.list[i].adminDeptName + '</TD>' +
                            '<TD>' + data.list[i].createUserName + '</TD>' +
                            '<TD>' + data.list[i].createTime + '</TD>' +
                            '<TD><a class="show_a" onclick="deleteAdminDeptInfo(this)" nid=' + data.list[i].id + '  uuid=' + data.list[i].userUuid + '>删除</a></TD>' +
                            '</TR>'
                        );
                    } else {
                        $("#adminDepartMainTable").append(
    ...
    function deleteAdminDeptInfo(obj) {
        var id = $(obj).attr("nid");
        var uuid = $(obj).attr("uuid");
        var url = "<%=path%>/adminDept/delete";
        var datas = {
            'umsAssignAdminDept.id': id,
            'umsAssignAdminDept.userUuid': uuid,
        };
        $.post(url, datas, function (data) {
            queryAdminDeptInfo(datas);
            if (data.success) {
...
function getUserStatus(){
    ...
    $.getJSON("/ums/code/codeListByType", {typeId: 10007}, function (data) {
        // $("#assessSerial").append($("<option>").attr({"value": ''}).text('请选择'));
        var html = '';

        for (var i = 0; i < data.length; i++) {
            userStatus[data[i].id] = data[i].codeName;
            html += "<input id='checkbox"+data[i].id+"' value='"+data[i].id+"' type='checkbox' name='userStatus' class='checkbox'/><label for='checkbox"+data[i].id+"'>"+data[i].codeName+"</label>"
        }
        $("#userStatus").html(html);
    });
}
function getAssessType(){
    //考核类型
    $.getJSON("/ums/code/codeListByType", {typeId: 10008}, function (data) {
        ...
        for (var i = 0; i < data.length; i++) {
            userStatus[data[i].id] = data[i].codeName;
            html += "<input id='radio"+data[i].id+"' value='"+data[i].id+"' type='radio' name='userInfo.assessType' class='radio'/><label for='radio"+data[i].id+"'>"+data[i].codeName+"</label>"
        }
