(function (e) {
    ...
    e.changeBackgroud = function (a) {
        $("div#out").html("pos: " + $(a).offset().top + "-------- name: " + $(a).attr("name"));
        $("#back_div").animate({scrollTop: $(a).offset().top}, 500);
...
$(function () {
    ...
    var h = f - 1;//f:节点数量，h：滚动的数量，减去1的话，最后就会留下一个不滚动，默认是减去row
    $(".list").height(110 * row);
    ...
    $(".arrowdown").click(function () {
        a && parseInt($(".list li:first").css("marginTop")) > -110 * h && (d++, b--, a = !1, $(".list li").eq(d - 1).liOut(), $(".list li").eq(d + row - 1).liIn(), $(".list li:first").animate({marginTop: 110 * b}, 600, "easeInOutQuad", function () {
            ...
            var vColor = $(".list li:eq(" + d + ")").find("a").attr("BColor");
            var vImg = $(".list li:eq(" + d + ")").find("a").attr("BImg");
            var aname = $(".list li:eq(" + d + ")").find("a").attr("name");

            //改变背景
            $("#back_div").animate({scrollTop: $("#back_div a:eq(" + d + ")").offset().top - $("#back_div a:eq(0)").offset().top}, 500);
            //$.changeBackgroud($("#back_div a[name='" + aname + "']"));
