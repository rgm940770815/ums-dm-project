...
@ParentPackage("base")
@Scope("prototype")
@Namespace("/Xml2Action")
@Results({
        @Result(name = "json", type = "json", params = {"root", "data"})
})
public class Xml2Action {
    ...
    //内设机构 deptNo
    private static final int deptNo1 = 7321;
    //直属机构
    private static final int deptNo2 = 7322;
    //院领导
    private static final int deptNo3 = 7323;
    //院领导 领导班子
    private static final int deptNo4 = 7324;
    //院领导 非领导班子
    private static final int deptNo5 = 7325;
...
//        }
        ...
        try {
            ...
            List<UmsUploadDeptContrast> umsUploadDeptContrasts = generateB01(map_query, court_map, umsDepartmentMap,null);
            String path_g = path + "/B01";
            Document doc = DocumentHelper.createDocument().addComment("");
            Element dataset = doc.addElement("dataset");
            //使用 addElement() 方法创建根元素 <root> 。 addElement() 用于向 XML 文档中增加元素。
...
//        ywlbMap.put("办公室","14");
    ...
    public List<UmsUploadDeptContrast> generateB01( Map<String, Map<String, String>> map_query , Map<String, Object> court_map  , Map<String,UmsDepartment> umsDepartmentMap ,List<String> partys){
        //用于查询的map 以 getCourtNo 为key  里面的map 以 deptNo 为key  转换后部门的B00 为value
//        Map<String, Map<String, String>> map_query = new HashMap<>();
//        court_map 以ums_court_full 的court_no 为key 以 转换后法院的B00 为 value
//        Map<String, Object> court_map = new HashMap<>();
...
//        Map<String,UmsDepartment> umsDepartmentMap = new HashMap<>();
        ...
        for (UmsCourtFull umsCourtFull : level0) {
            ...
            readDeptNode(umsCourtFull,map_query , court_map  , umsDepartmentMap , re , 0,partys);
        }
    ...
    private void readDeptNode(UmsCourtFull umsCourtFull,Map<String, Map<String, String>> map_query , Map<String, Object> court_map  , Map<String,UmsDepartment> umsDepartmentMap,List<UmsUploadDeptContrast> re,Integer level,List<String> partys){
        ...
        if(childCourtList != null && childCourtList.size() > 0){
            ...
            re.add(contrast);
            if(!partys.contains(uuid)){
                updateInstitution(umsCourtFull,deptNo1,uuid);
            }
        }
        ...
        if(level == 0){
            ...
            if(!partys.contains(uuid_zsjg)){
                updateInstitution(umsCourtFull,deptNo2,uuid_zsjg);
            }
        }
        ...
        for (UmsDepartment department : l) {
            ...
            if (department.getDeptName().equals("院领导")) {
                ...
                String uuid1 = getUUID(umsCourtFull.getCourtCode() + 2);
                u1.setId(uuid1);
                ...
                if(!partys.contains(uuid1)){
                    updateInstitution(umsCourtFull,deptNo4,uuid1);
                }

                UmsDepartment u2 = new UmsDepartment();
                //u2.setId(department.getCourtNo() + "" + fbzcyDeptNo);
                String uuid2 = getUUID(umsCourtFull.getCourtCode() + 3);
                u2.setId(uuid2);
                ...
                if(!partys.contains(uuid2)){
                    updateInstitution(umsCourtFull,deptNo5,uuid2);
                }
            ...
            //查找是否有子部门
            addChild(re,innerMap,umsDepartmentMap,umsCourtFull.getCourtCode(), department.getId(), sortNo,partys);
        ...
        if(childCourtList != null && childCourtList.size() > 0){
            ...
            for (UmsCourtFull courtFull : childCourtList) {
                ...
                readDeptNode(courtFull,map_query ,court_map  , umsDepartmentMap , re, level,partys);
    ...
    private void addChild(List<UmsUploadDeptContrast> re, Map<String, String> innerMap, Map<String, UmsDepartment> umsDepartmentMap, String courtCode, String pid, String pindex,List<String> partys){
        ...
        for (UmsDepartment department : l) {
            ...
            addChild(re , innerMap, umsDepartmentMap, courtCode , department.getId(), umsUploadDeptContrast.getPindex(),partys);
...
//        court_map 以ums_court_full 的court_no 为key 以 转换后法院的B00 为 value
        ...
        List<UmsUploadDeptContrast> umsUploadDeptContrasts = generateB01(map_query, court_map, umsDepartmentMap,partys);

        List<String> ok = new ArrayList<>();
        List<UmsUploadDeptContrast> out = new ArrayList<>();
        //法院
        UmsCourtFullCriteria criteria = new UmsCourtFullCriteria();
        criteria.clear();
        ...
        for (UmsUploadDeptContrast umsUploadDeptContrast : umsUploadDeptContrasts) {
            ...
            if(StringUtils.hasText(b00) && partys.contains(b00)){
                ok.add(b00);
            }else{
                out.add(umsUploadDeptContrast);
                //更新
    ...
    public void updateInstitution(UmsCourtFull umsCourtFull,Integer departmentNo,String institutionId){
        ...
        String o = "0";
        information.setIsBuild(o);
        information.setDeptNo(departmentNo);
        information.setInstitutionId(institutionId);
